	...
	function start() {
		...
		$(window).trigger('start');
                function generateArrows() {
                  ...
                  var image = new Image();
                  image.setAttribute('src', 'images/arrow.png');
                  var dir = Math.round(Math.random() * 3);
                  var orientations = ['leftarrow', 'rightarrow', 'uparrow', 'downarrow'];
                  $(arrow).addClass(orientations[dir]);
                  ...
                  arrow.addEventListener('transitionend', transitionEnd, false);
                  arrow.addEventListener('webkitTransitionEnd', transitionEnd, false);
                  setTimeout(generateArrows, Math.round(Math.random() * 1500));
                };
                generateArrows();
		update();
	}
...
contextSource.drawImage(video, 0, 0);
	...
	function highlightHotSpots() {
        ...
        function closeBy(e1, e2, tolerance) {
          var offset1 = e1.position();
          var offset2 = e2.position();
          return Math.abs(offset1.left - offset2.left) < tolerance &&
                 Math.abs(offset1.top - offset2.top) < tolerance;
        }
   	...
   	$(window).on('motion', function(ev, data){
		var spot = $(data.spot.el);
                ...
                for (var i = 0; i < arrows.length; i++) {
                  var arrow = arrows[i];
                  if (closeBy($(arrow), spot, 10)) {
                    arrow.parentNode.removeChild(arrow);
                    window.score += 50;
                    $(document.body).removeClass("black-background");
                    setTimeout(function() {
